...
void setup() {
  ...
  // * Copy all EEPROM Values into Array (direct EEPROM accces time is too long) *
  ...
  for (int i = 0; i < sidCount; i++) {
    ...
    // Volume
    sidChip[i].setreg(24, 15);

    // Waveform Recangle (PCM)
    sidChip[i].setreg(18, 64);
    sidChip[i].setreg(11, 64);
    sidChip[i].setreg(4, 64);

    // PCM Duty cycle
    sidChip[i].setreg(17, 8);
    sidChip[i].setreg(10, 8);
    sidChip[i].setreg(3, 8);

    // * Hold/Release *
    sidChip[i].setreg(20, 208);
    sidChip[i].setreg(13, 208);
    sidChip[i].setreg(6, 208);
  }
  ...
  if (rw_state == LOW) {
    ...
    */
    ...
    if (regAddr >= 0 && regAddr <= 24) {  // SID Registers 0-24;
      ...
      for (int i = 0; i < sidCount; i++) {
        if (busAdr == sidAddresses[i]) {
          sidChip[i].setreg(regAddr, data_val);
        }
      }
    }
